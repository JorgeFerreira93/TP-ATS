...
public class Main {
	...
	public static void main(String[] args) {
		try {
			...
			try {
				...
				counterMap.put("Func", 0);
				counterMap.put("Var", 0);
				//Instrucao p3 = `TopDown(stratBadSmells()).visit(p);
				//`TopDown(countFunct(counter)).visit(p);
				`TopDown(countFunct(counterMap)).visit(p);
				System.out.println("Funcs: " + counterMap.get("Func") + "\nVars: " + counterMap.get("Var"));
				/*System.out.println(numInstrucao.toString());
    ...
    %strategy countFunct(counter : HashMap)extends Identity(){
		visit Instrucao {
			Funcao(_,tipo,_,nome,_,_,argumentos,_,_,instr,_) -> {
				int aux = (int)counter.get("Func");
				counter.put("Func", aux+1);
				System.out.println(contaArgumentos(`argumentos));
			}

			Declaracao(_,tipo,_,decls,_,_) -> {
				int aux = (int)counter.get("Var");
				counter.put("Var", aux+1);
			}
		}
	}
    ...
    private void teste(Instrucao inst){
    	teste2(inst);
    	System.out.println("Nº de funções: " + this.nFuncs);
    }

    private void teste2(Instrucao inst){
    	System.out.println("dsahudhsa");
    	%match(inst){
    		Atribuicao(_,id,_,opAtrib,_,exp,_) -> {
